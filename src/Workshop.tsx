...
export function WorkshopNav({ active }: { active?: WorkshopScreen } = {}) {
  ...
  return <nav className="ws-nav" aria-label="局外导航">{items.map(item => <button type="button" key={item.id} data-testid={item.testId} className={current === item.id ? 'is-active' : ''} onClick={() => go(item.id)}><Icon name={item.icon} size={19} /><span>{item.label}</span></button>)}</nav>;
}
...
function PageHeading({ eyebrow, title, subtitle, back, backLabel = '返回餐厅', onBack }: { eyebrow: string; title: string; subtitle?: string; back?: boolean; backLabel?: string; onBack?: () => void }) {
  return <header className="ws-heading">
    ...
    <div><p className="ws-eyebrow">{eyebrow}</p><h1>{title}</h1>{subtitle && <p className="ws-subtitle">{subtitle}</p>}</div>
  </header>;
}
...
function Restaurant({ meta, go }: { meta: MetaState; go: (screen: WorkshopScreen) => void }) {
  ...
  return <section className="ws-screen ws-screen--restaurant" data-testid="workshop-restaurant">
    ...
    <div className="ws-restaurant-hero"><p className="ws-eyebrow">深渊餐厅 · 今夜营业</p><h1>把战利品，<br /><em>做成传说。</em></h1><p>精准招架，收集食材，以一桌热气腾腾的料理换取下一场冒险。</p><ActionButton testId="explore" onClick={() => go('stages')}>前往探险 <Icon name="arrow" size={19} /></ActionButton></div>
    <div className="ws-quick-grid"><button type="button" onClick={() => go('kitchen')}><span className="ws-quick-icon"><Icon name="pan" size={24} /></span><span><b>厨房</b><small>{ingredientCount ? `${ingredientCount} 份食材可用` : '等待新鲜食材'}</small></span><Icon name="arrow" size={17} /></button><button type="button" onClick={() => go('sales')}><span className="ws-quick-icon ws-quick-icon--gold"><Icon name="shop" size={24} /></span><span><b>菜架</b><small>{meta.pendingDishes.length ? `${meta.pendingDishes.length} 道料理待售` : '还没有待售料理'}</small></span><Icon name="arrow" size={17} /></button></div>
...
function Shop({ meta, go, buyItem, equipItem }: { meta: MetaState; go: (screen: WorkshopScreen) => void; buyItem: (id: string) => Promise<boolean> | boolean; equipItem: (id: string | null) => Promise<boolean> | boolean }) {
  ...
  return <section className="ws-screen ws-screen--shop" data-testid="workshop-shop"><div className="ws-content"><PageHeading eyebrow="THE PROVISIONER" title="商店" subtitle="把战斗赚来的金币，换成更锋利的下一步。" back onBack={() => go('restaurant')} /><div className="ws-shop-balance"><span>钱包余额</span><CoinPill coins={meta.coins} /><small>唯一可购买装备会自动解锁，预览物品将在后续版本开放。</small></div><div className="ws-shop-grid"><ShopCard item={{ id: 'base_knife', name: '基础厨刀', category: 'weapon', cost: 0, implemented: true, verdictDamageBonus: 0, description: '可靠的起点，没有额外加成' }} equipped={!meta.equippedWeaponId} owned onAction={() => run(() => equipItem(null))} actionLabel="已装备" /><>{SHOP_ITEMS.map(item => { const owned = meta.purchasedShopItemIds.includes(item.id); const equipped = item.category === 'weapon' && meta.equippedWeaponId === item.id; const canBuy = meta.coins >= item.cost && item.implemented && !owned; return <ShopCard key={item.id} item={item} owned={owned} equipped={equipped} disabled={!item.implemented || (!owned && !canBuy)} onAction={() => run(() => owned ? equipItem(item.id) : buyItem(item.id))} actionLabel={equipped ? '已装备' : owned ? '装备' : item.implemented ? `购买 · ${item.cost}` : '即将开放'} />; })}</></div></div><WorkshopNav active="shop" /></section>;
}

function ShopCard({ item, owned, equipped, disabled, onAction, actionLabel }: { item: ShopItemDefinition; owned: boolean; equipped: boolean; disabled?: boolean; onAction: () => void; actionLabel: string }) {
  return <article className={`ws-shop-card ${item.implemented ? '' : 'is-preview'} ${equipped ? 'is-equipped' : ''}`}><div className={`ws-shop-art ws-shop-art--${item.category}`}><Icon name={item.category === 'weapon' ? 'sword' : item.category === 'skin' ? 'spark' : 'bag'} size={29} />{!item.implemented && <span>PREVIEW</span>}</div><div className="ws-shop-copy"><span className="ws-panel-kicker">{item.category === 'weapon' ? 'WEAPON' : item.category === 'skin' ? 'OUTFIT' : 'SUPPLY'}</span><h2>{item.name}</h2><p>{item.description}</p>{item.verdictDamageBonus > 0 && <strong className="ws-bonus">+{item.verdictDamageBonus} 裁决伤害</strong>}</div><ActionButton mutates variant={item.implemented ? 'primary' : 'quiet'} disabled={disabled || equipped} onClick={onAction}>{equipped ? <><span className="ws-check">✓</span> 已装备</> : owned ? '装备' : actionLabel}</ActionButton></article>;
}
