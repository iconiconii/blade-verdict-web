...
export function makeBattleCamera(width:number,height:number){
  ...
  const distance=Math.max(7.7,6/aspect);
  // Reserve room for a 90px body ring and the fixed left energy vial.
  camera.position.set(0,1.75,distance);camera.lookAt(aspect<.8?-.22:0,aspect<.8?1.48:1.25,0);camera.updateMatrixWorld(true);return camera;
}
