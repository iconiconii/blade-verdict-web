...
function Result(){
  ...
  const jelly=useGame(s=>s.combat.bossKind==='jelly');
  return <section className="page result"><div className="result-card"><small>{win?'HUNT COMPLETE':'EXPEDITION FAILED'}</small><h1>{win?'讨伐成功':'败北'}</h1><div className="seal">{win?'完':'败'}</div><p>{win?'刀锋之下，收获已定。':'调整呼吸，等待光环收拢。'}</p><p>最高连击 {bestCombo} · 裁决 {battle.verdictCount} 次</p>{win&&battle.loot.length>0?<div className="loot"><span>{jelly?'🫐':'🌽'}</span><div><b>{jelly?'果冻核':'玉米粒'} · {battle.loot.reduce((sum,item)=>sum+item.count,0)} 份</b>{battle.loot.map((item,i)=><small key={i}>{qualityLabels[item.quality]} × {item.count}</small>)}</div></div>:<p className="empty-loot">{win?'本次没有切割食材':'本次未带回食材'}</p>}<button data-testid="home" onClick={()=>useGame.getState().go('restaurant')}>返回餐厅</button></div></section>;
}
