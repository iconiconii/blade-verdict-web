...
export function Battle(){
  ...
  const {battle,phase,targets,feedback,combo,paused}=combat;
  ...
  useEffect(()=>{
    ...
    try{scene.current=new BattleScene(host.current!,error,()=>{if(alive){readyRef.current=true;setReady(true);clock.current=performance.now()}})}catch{error()}
  ...
  return <section className={`battle battle--${phase} ${feedback?.kind==='Miss'&&showFeedback?'battle--hurt':''}`} data-testid="battle" data-battle-phase={phase}>
    ...
    <header className="battle-top">
      ...
      <div className="boss-hud"><div className="boss-heading"><span>NO. 01 / CORN GUARDIAN</span><span>危险度 II</span></div><h2>暴怒玉米守卫 <small>{phaseLabels[phase]}</small></h2><div className="battle-health boss-health"><i style={{width:`${battle.bossHp/8}%`}}/></div><div className="boss-meta"><span>荒芜农场 · 月下讨伐</span><span data-testid="boss-hp">{battle.bossHp} <em>/ 800</em></span></div></div>
      <button className="battle-icon" aria-label="暂停战斗" onClick={()=>useGame.getState().pause(true)}>Ⅱ</button>
    </header>
    <div className="phase-ribbon"><span className="phase-dot"/>{inVerdict?'BREAK · 破防时刻':targets.length===2?'DOUBLE · 双重攻势':'PARRY · 观察 · 等待 · 反击'}</div>
    ...
    <footer className="battle-bottom">
      ...
      <div className="combat-instruction">{phase==='verdictSlash'?'沿发光路径划过弱点':phase==='verdictReady'?'点击裁决徽章开始':phase==='telegraph'?'怪物正在蓄力':targets.some(t=>t.phase==='perfect')?'现在 · 击中核心':'金色 NICE · 青色 PERFECT'}</div>
